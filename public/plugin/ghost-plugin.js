...
function toggleNotificationDropdown() {
	...
	const notificationDropdown = document.getElementById(NOTIFICATION_DROPDOWN_ID);
...
function renderNotificationListItem(sseMessage) {
	...
	title.textContent = sseMessage.type === 'replace' ? 'Article updated' : 'Article published';
...
function initEventSource() {
	...
	subscription.addEventListener('replace', (ev) => {
		...
		document.getElementById(NOTIFICATION_LIST_ID)?.prepend(renderNotificationListItem(ev));
