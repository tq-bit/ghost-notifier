...
export default {
  handleArticleCreationNotification: async (req: Request, res: Response) => {
    try {
      ...
      logger.verbose(
        `Notification created for published article ${incomingArticle.id}`,
      );
  ...
  handleArticleUpdateNotification: async (req: Request, res: Response) => {
    try {
      ...
      logger.verbose(
        `Notification created for updated article ${incomingArticle.id}`,
      );
  ...
  handleArticleScheduleNotification: async (req: Request, res: Response) => {
    try {
      ...
      NotificationModel.createNotification(notification);
      logger.verbose(
        `Notification created for scheduled article ${incomingArticle.id}`,
      );
