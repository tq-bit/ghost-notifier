...
type AppWebhookGenerationResponse = [
  null | AxiosError,
  null | GhostWebhookGenerationResponse,
];
...
export default {
  ...
  async postGenerateWebooks({
    ...
    try {
      ...
      const postPublishedResult = await axios.post(
        url,
        payloadPostPublished,
        config,
      );
      const postPublishedEditedResult = await axios.post(
        url,
        payloadPostPublishedEdited,
        config,
      );
      const postScheduledResult = await axios.post(
        url,
        payloadPostScheduled,
        config,
      );
