...
export default {
  ...
  validateDomainStatus: async (
    ...
    try {
      ...
      if (domain.status === 'inactive') {
        throw new NotPermittedError(
          `Domain with id ${domainId} (${domain.name}) is not active!`,
        );
      }
    ...
    try {
      ...
      if (!domainOwner) {
        throw new NotAuthorizedError(
          `Request to validate domain owner was not authorized!`,
        );
      }
      ...
      if (!domain) {
        throw new NotFoundError(
          `Domain with id ${domainId} and owner ${domainOwner} not found!`,
        );
      }
  ...
  validateDomainWebhookPayload: async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
