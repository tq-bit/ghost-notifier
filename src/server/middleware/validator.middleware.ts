...
export default {
	...
	validateSuperUserWasCreated: async (req: Request, res: Response, next: NextFunction) => {
		try {
			const appConfig = await AppConfig.readAppConfig();
	...
	validateFirstApplicationVisit: async (req: Request, res: Response, next: NextFunction) => {
		try {
			const appConfig = await AppConfig.readAppConfig();
			if (!appConfig.wasSuperUserCreated) {
			...
			if (error instanceof NotFoundError) {
				...
				return new Responder(req.headers['content-type'] || 'text/html', {
					onJson: () => res.status(code).send({ status: GN_WARNING_STATUS, error: message }),
					onOther: () => res.redirect(`/signup/su?status=${GN_WARNING_STATUS}&message=${message}`),
				}).send();
			}

			logger.error(error);
			return new Responder(req.headers['content-type'] || 'text/html', {
				onJson: () => res.status(500).send({ status: GN_WARNING_STATUS, error: error }),
				onOther: () => res.redirect(`/signup/su?status=${GN_WARNING_STATUS}&message=${error}`),
			}).send();
		}
	},
};
